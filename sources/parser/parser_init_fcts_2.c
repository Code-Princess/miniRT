...
void	init_axis_vec(t_object *obj, char **line_arr, int idx)
{
	...
	exit_if_args_incomplete(line_arr, idx, "axis_vec: not enough arguments");
	x_normal_vec = ft_atof(line_arr[idx]);
	exit_if_args_incomplete(line_arr, idx + 1, "axis_vec: not enough arguments");
	y_normal_vec = ft_atof(line_arr[idx + 1]);
	exit_if_args_incomplete(line_arr, idx + 2, "axis_vec: not enough arguments");
	z_normal_vec = ft_atof(line_arr[idx + 2]);
	vec = set_tuple(x_normal_vec, y_normal_vec, z_normal_vec, VEC);
	if (is_normalized(&vec))
	...
	{
		print_error_and_exit("axis_vec: vector not normalized", line_arr[0]);
	}
}
...
void	init_angle(t_object *obj, char **line_arr, int idx)
{
	...
	exit_if_args_incomplete(line_arr, idx, "angle: not enough arguments");
...
void	init_radius(t_object *obj, char **line_arr, int idx)
{
	...
	exit_if_args_incomplete(line_arr, idx, "radius: missing argument");
...
void	init_height(t_object *obj, char **line_arr, int idx)
{
	...
	exit_if_args_incomplete(line_arr, idx, "height: missing argument");
