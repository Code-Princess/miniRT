...
void	init_position(t_object *obj, char **line_arr, int idx)
{
	...
	if (is_in_range_float(&x_coord, -10000, 10000) && is_in_range_float(&y_coord, -10000, 10000) && \
	...
	else
	{
		...
		print_error_and_exit("position: value not in range [-10000,10000]", line_arr[0], obj);
	}
}
