...
/*   Updated: 2024/12/05 16:29:27 by llacsivy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_tuple	calc_origin2(t_object *camera, t_tuple orthonormal_vec_x, \
...
void	init_image_plane(t_object *cam)
{
	...
	{
		orthonormal_vec_x = tuple_normalize2(&(t_tuple){1, 0, 0, 0});
		orthonormal_vec_y = tuple_normalize2(&(t_tuple){0, 0, 1, 0});
	}
	else
	{
		temp = tuple_cross2(&cam->s_camera.normal_vec, &(t_tuple){0, -1, 0, 0});
		orthonormal_vec_x = tuple_normalize2(&temp);
		temp = tuple_cross2(&cam->s_camera.normal_vec, &orthonormal_vec_x);
		orthonormal_vec_y = tuple_normalize2(&temp);
	}
	pixel_len = CANVAS_WIDTH / WIDTH_IN_PIXEL;
	cam->s_camera.s_img_plane.delta_x_vec = tuple_scale2(pixel_len, \
													&orthonormal_vec_x);
	cam->s_camera.s_img_plane.delta_y_vec = tuple_scale2(pixel_len, \
													&orthonormal_vec_y);
	cam->s_camera.s_img_plane.origin = calc_origin2(cam, \
